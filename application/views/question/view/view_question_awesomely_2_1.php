...
<h4>Is the implementation of tool <?= $tool->tool_no . ' : ' . $tool->tool_title . ' ' ?>complete ?    
    
    <input class="magic-radio radio-inline tool_check" id="tool<?= $tool->tool_no ?>_yes_check" type="radio" name="question_enable_tool<?= $tool->tool_no ?>" value="true"  <?= $disabled ?>><label>Yes</label> <!-- temporary remove required -->
    <input class="magic-radio radio-inline tool_check" id="tool<?= $tool->tool_no ?>_no_check" type="radio" name="question_enable_tool<?= $tool->tool_no ?>" value="false"  <?= $disabled ?>><label>No</label>
    
    <small>   <?= $tool->tool_justification ? 'Justification :' . $tool->tool_justification : ' ' ?></small></h4>
...
<table class="table table-bordered table-hover">
    <thead>
    <th>Subject</th><th> Status </th><th> Justification </th>
    </thead>
    <!--
...
<img src="<?= base_url() ?>theme1/assets/css/ajax-loader.gif" id="img" style="display:none" />
                                        ...
                                        <tr>
                                            <td style="width: 25%">
                                                <h4><b>Date of Implementation : </b>
                                                    <input class='form-control' type='date' name='date_implement#<?= $tool->id ?>' max='<?= date('Y-m-d') ?>'placeholder='select date' <?= $disabled ?>><?php
//                        $data = array(
//                            'class' => 'form-control',
...
<script>
    ...
    $("#myForm").submit(function (e) {
        ...
        $('#save_btn, #submit_btn').one("click", function (event) {
            if (!this.submitting)
            {
                ...
                for (var key in data) {
                    if (data.hasOwnProperty(key)) {
                        ...
                        if (data[key].tool1_date_implement === '0000-00-00') {
                            $('[name="date_implement#1"]').val('0000-00-00');
                        } else {
