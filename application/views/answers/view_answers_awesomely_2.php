...
$tool1_field = substr($field_name[16], 4, 1); //tool 1 implementation 
$tool2_field = $field_name[17]; //tool 1 implementation 
$tool3_field = $field_name[18]; //tool 1 implementation 
$tool4_field = $field_name[19]; //tool 1 implementation 
$tool5_field = $field_name[20]; //tool 1 implementation 
$tool6_field = $field_name[21]; //tool 1 implementation 
$tool7_field = $field_name[22]; //tool 1 implementation 

echo 'tool1 field name - '.  $tool1_field;
?>
                                        ...
                                        foreach ($questions_title as $titles):
                                            ...
                                            foreach ($titles as $title) {
                                                if ($tool->tool_no == $title->id_tool) {
                                                    ...
                                                    <?php
                                                    if ($rules_applied) {
                                                        if ($title->id_categories == 'A') {
                                                            echo '<tr><td></td><td colspan="5" style="background-color: #ccffcc;">';
                                                            echo '<b>INDUSTRIAL EFFLUENT - 2009 , SEWAGE - 2009, CRUDE PALM OIL - 1977</b>';
                                                            echo '</td></tr>';
                                                        } if ($title->id_categories == 'B') {
                                                            echo '<tr><td></td><td colspan="5" style="background-color: #ccffcc;">';
                                                            echo '<b>SCHEDULE WASTE - 2005</b>';
                                                            echo '</td></tr>';
                                                        } if ($title->id_categories == 'C') {
                                                            echo '<tr><td></td><td colspan="5" style="background-color: #ccffcc;">';
                                                            echo '<b>CLEAN AIR - 2014</b>';
                                                            echo '</td></tr>';
                                                        }
                                                    }
        ...
        $("#update_verification").click(function (e) {
            ...
            if (isChecked) {
                $.ajax({
                    ...
                    data: {
                        verify_type: $("[name=verify_type]:checked").val()
                    },
...
//                });
        $('#smartwizard').smartWizard({
            ...
            toolbarSettings: {toolbarPosition: 'both',
                ...
                toolbarExtraButtons: [btnDoeFinish]
            },
            anchorSettings: {
    ...
    $("#myForm").submit(function (e) {
        ...
        var confirmDialog = confirm("Do you want to submit this score?");
...
//                $("#download_modal").modal("show");
                    window.setTimeout(function () {
                        //
                        //$.notify("Review has been submitted, this page will now refresh");
                        $.notify({
                            message: "Review has been submitted, this page will now refresh"
                        },
                                {
                                    placement: {
                                        from: "top",
                    ...
                    window.setTimeout(function () {
                        //
                        window.location.href = '<?php echo site_url('dashboard/#cTotalRegistered') ?>';
    ...
    function populate_form(idpremis)
    {
        ...
        //Ajax Load data from ajax
        $.ajax({
            url: "<?php echo site_url('questions/get_tool_submission_status/') ?>" + idpremis + '/' + <?= $emt_id ?>,
            type: "GET",
            dataType: "JSON",
            success: function (data)
            {
                for (var key in data) {
                    if (data.hasOwnProperty(key)) {
                        ...
                        if (data[key].tool7_implementation === 'false') {
                            ...
                            disable_input(7);
                            $('#tool_no_justification7').attr("disabled", true).addClass('ui-state-disabled');
                        }
                        if (data[key].tool1_implementation === 'false') {
                            ...
                            disable_input(1);
                            $('#tool_no_justification1').attr("disabled", true).addClass('ui-state-disabled');
                        }
                        if (data[key].tool2_implementation === 'false') {
                            ...
                            disable_input(2);
                            $('#tool_no_justification2').attr("disabled", true).addClass('ui-state-disabled');
                        }
                        if (data[key].tool3_implementation === 'false') {
                            ...
                            disable_input(3);
                            $('#tool_no_justification3').attr("disabled", true).addClass('ui-state-disabled');
                        }
                        if (data[key].tool4_implementation === 'false') {
                            ...
                            disable_input(4);
                            $('#tool_no_justification4').attr("disabled", true).addClass('ui-state-disabled');
                        }
                        if (data[key].tool5_implementation === 'false') {
                            ...
                            disable_input(5);
                            $('#tool_no_justification5').attr("disabled", true).addClass('ui-state-disabled');
                        }
                        if (data[key].tool6_implementation === 'false') {
                            ...
                            disable_input(6);
                            $('#tool_no_justification6').attr("disabled", true).addClass('ui-state-disabled');
                        }
