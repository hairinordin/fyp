...
<?php
        ...
        $tool4_DOE_score2 = $tool4_DOE_score ? ($tool4_DOE_score * $tool_weightage[3]->tool_full_score) / 100 : '0';
        $tool5_DOE_score2 = $tool5_DOE_score ? ($tool5_DOE_score * $tool_weightage[4]->tool_full_score) / 100 : '0';
        $tool6_DOE_score2 = $tool6_DOE_score ? ($tool6_DOE_score * $tool_weightage[5]->tool_full_score) / 100 : '0';
...
<div class="tab-content">
    ...
    <?= form_open('answers/submitBaru/' . $tool->tool_no , 'id="form'.$tool->tool_no.'"') ?>
                        ...
                        if ($tool->tool_no == '2' || $tool->tool_no == '7') {
                            ...
                            <?php if ($quest->id_question_title != NULL && $quest->id_question_title == $title->id) { ?>
                                ...
                                <?php if (($quest->id == 120) || ($quest->id == 121) || ($quest->id == 122)) { ?>
                                                                                <select class="form-control" name='scoreDOE<?= $quest->id ?>' onchange="changeScore(<?= $quest->id ?>, this.value)">
    ...
    ?>
                ...
                <div class="row">
                    <div class="col-md-7">
                    <div class="form-group">
                        <h4>Comment on tool (if any) 
<!--                        <textarea id="textarea_editor<?= $tool->tool_no?>"  style="width:100%" rows="5" placeholder="Comment on tool (optional)" name="comment"> -->
                            <?php
                    ...
                    <div class="col-md-5">
                        <div class="form-group">
                            <h4>
                                ...
                                //print_r($tool3_review)
                                switch($tool->tool_no){
                                    case 1: echo isset($tool1_DOE_score)? $tool1_DOE_score:''; 
                                        break;
                                    case 2: echo isset($tool2_DOE_score)? $tool2_DOE_score:''; 
                                        break;
                                    case 3: echo isset($tool3_DOE_score)? $tool3_DOE_score:''; 
                                        break;
                                    case 4: echo isset($tool4_DOE_score)? $tool4_DOE_score:''; 
                                        break;
                                    case 5: echo isset($tool5_DOE_score)? $tool5_DOE_score:''; 
                                        break;
                                    case 6: echo isset($tool6_DOE_score)? $tool6_DOE_score:''; 
                                        break;
                                    case 7: echo isset($tool7_DOE_score)? $tool7_DOE_score . '.00':''; 
                                        break;
                                    default : echo ''; break;          
                                }
                            ?> / 100.00
...
<!--                    <button class="btn btn-lg" type="submit"><i class="fa fa-floppy-o fa-fw" aria-hidden="true"></i> Save Asessment</button>
...
//                        });
    ...
    $(document).ready(function () {
        ...
        $("#form4 :input").prop("disabled", true);
        $("#form5 :input").prop("disabled", true);
        $("#form6 :input").prop("disabled", true);
        $("#form7 :input").prop("disabled", true);
