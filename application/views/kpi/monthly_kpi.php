...
<h3>Monthly KPI for the year of <?= $year ?> <small> <?= $negeri ?> </small></h3>
<hr>
...
<?php if ($this->role == 'KPI'): ?>
    <div class="row">
        ...
        <div class="col-md-6 pull-right">
            ...
            <br>
            <button class="btn btn-success btn-lg" onclick="verify_submit()" <?= ($kpi_status) ? 'disabled' : '' ?>>Approve State KPI</button>
        </div>
    </div>
<?php endif; ?>
        ...
        <div class="col-md-2 col-xs-2">
            <div class="form-group">
                ...
                <select class="form-control" name="year" id="sel1">
                    ...
                    <option value="2020" <?= $selected20 ?>>2020</option>
                    <option value="2021" <?= $selected21 ?>>2021</option>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" name="state" id="sel2">
                        ...
                        foreach ($state_kpi_status as $key => $value) {
                            if ($key != 16) {
                                ?>
                                <option value="<?= $key ?>" <?= ($value['status']) ? '' : 'disabled' ?>><?= $value['negeri'] ?></option>
                                <?php
                            }
                        }
                        ?>
...
<table class='table table-bordered'>
    ...
    <?php } ?>
                ...
                <td>
                    <button type="button" class="btn" data-toggle="collapse" data-target=".collapse-kks">
                        Click to expand
                    </button>
                </td>
            </tr>
            <tr id="" class="collapse out collapse-kks officer-head-col">
    ...
    <?php foreach ($officers as $off): ?>
                <tr id="" class="collapse out collapse-kks officer-col">
                    <td colspan="2">
                        <?= ((($this->role == 'KPI') && ($flag_february == 1) && (!$kpi_status)) || ($this->role == 'KPI' && $kpi_status == 0)) ? '<a href="' . base_url("kpi/edit_monthly/") . $off->id . '/41">Edit</a> | ' : '' ?>

                    <?= $off->name ?> 
                    </td>
                    <?php
                    if ($off->kks) {
                        foreach ($off->kks as $kks) {  // Loop 12 bulan       
                            ?>
                            <td><?= (!$kks->sasaran_bulan) ? '0' : $kks->sasaran_bulan ?></td>
        ...
        <?php } ?>
                    ...
                    <td><?= $off->total_kks ?> </td>
                    <td>Submitted : <?= $off->kks_submitted ? date("Y/m/d H:i:s", strtotime($off->kks_submitted)) : '-' ?> 
                        <br>
                        Updated : <?= $off->kks_updated ? date("Y/m/d H:i:s", strtotime($off->kks_updated)) : '-' ?> 
                    </td>
                </tr>
        <?php endforeach; ?>
        </tbody>
    ...
    <?php } ?>
                ...
                <td > 
                    <button type="button" class="btn" data-toggle="collapse" data-target=".collapse-kg">
                        Click to expand
                    </button>
                </td>
            </tr>
            <tr id="" class="collapse out collapse-kg officer-head-col">
                <td colspan="2"> Officer </td>
                <td>January</td>
                <td>February</td>
                <td>March</td>
    ...
    <?php foreach ($officers as $off): ?>
                <tr id="" class="collapse out collapse-kg officer-col">
                    <td colspan="2"> 
                        <?= ((($this->role == 'KPI') && ($flag_february == 1) && (!$kpi_status)) || ($this->role == 'KPI' && $kpi_status == 0)) ? '<a href="' . base_url("kpi/edit_monthly/") . $off->id . '/40">Edit</a> | ' : '' ?>
                    <?= $off->name ?> 
                    </td>
                    <?php
                    if ($off->kg) {
                        foreach ($off->kg as $kg) {  // Loop 12 bulan       
                            ?>
                            <td><?= (!$kg->sasaran_bulan) ? '0' : $kg->sasaran_bulan ?></td>
        ...
        <?php } ?>
                    ...
                    <td><?= $off->total_kg ?> </td>
                    <td>Submitted : <?= $off->kg_submitted ? date("Y/m/d H:i:s", strtotime($off->kg_submitted)) : '-' ?> 
                        <br>
                        Updated : <?= $off->kg_updated ? date("Y/m/d H:i:s", strtotime($off->kg_updated)) : '-' ?> 
                    </td>
                </tr>
        <?php endforeach; ?>
        </tbody>
    ...
    <?php } ?>
                ...
                <td > 
                    <button type="button" class="btn" data-toggle="collapse" data-target=".collapse-bt">
                        Click to expand
                    </button>
                </td>
            </tr>
            <tr id="" class="collapse out collapse-bt officer-head-col">
                <td colspan="2"> Officer </td>
                <td>January</td>
                <td>February</td>
                <td>March</td>
    ...
    <?php foreach ($officers as $off): ?>
                <tr id="" class="collapse out collapse-bt officer-col">
                    <td colspan="2"> 
                        <?= ((($this->role == 'KPI') && ($flag_february == 1) && (!$kpi_status)) || ($this->role == 'KPI' && $kpi_status == 0)) ? '<a href="' . base_url("kpi/edit_monthly/") . $off->id . '/42">Edit</a> | ' : '' ?>
                    <?= $off->name ?> 
                    </td>
                    <?php
                    if ($off->bt) {
                        foreach ($off->bt as $bt) {  // Loop 12 bulan       
                            ?>
                            <td><?= (!$bt->sasaran_bulan) ? '0' : $bt->sasaran_bulan ?></td>
        ...
        <?php } ?>
                    ...
                    <td><?= $off->total_bt ?> </td>
                    <td>Submitted : <?= $off->bt_submitted ? date("Y/m/d H:i:s", strtotime($off->bt_submitted)) : '-' ?> 
                        <br>
                        Updated : <?= $off->bt_updated ? date("Y/m/d H:i:s", strtotime($off->bt_updated)) : '-' ?> 
                    </td>
                </tr>
        <?php endforeach; ?>  
        </tbody>
...
<table class='table table-bordered'>
    ...
    <tbody>
        ...
        <?php } ?>
                    ...
                    <td> <button type="button" class="btn" data-toggle="collapse" data-target=".collapse-<?= $sic->SIC ?>">
                            Click to expand
                        </button>
                    </td>
                </tr>
                <tr id="" class="collapse out collapse-<?= $sic->SIC ?> officer-head-col">
                    <td colspan="2"> Officer </td>
                    <td>January</td>
                    <td>February</td>
                    <td>March</td>
        ...
        <?php foreach ($officers as $off): ?>
                    ...
                    <tr id="" class="collapse out collapse-<?= $sic->SIC ?> officer-col">
                        <td colspan="2">
                            <?= ((($this->role == 'KPI') && ($flag_february == 1) && (!$kpi_status)) || ($this->role == 'KPI' && $kpi_status == 0)) ? '<a href="' . base_url("kpi/edit_monthly/") . $off->id . '/' . $code . '">Edit</a> | ' : '' ?>
                        <?= $off->name ?> 
                        </td>
                        <?php
                        if ($off->$code) {
                            foreach ($off->$code as $c_sic) {  // Loop 12 bulan       
                                ?>
                                <td><?= (!$c_sic->sasaran_bulan) ? '0' : $c_sic->sasaran_bulan ?></td>
            ...
            <?php } ?>
                        ...
                        <td><?= $off->{'total_' . $sic->SIC} ?> </td>
                        <td>Submitted : <?= $off->{$sic->SIC . '_submitted'} ? date("Y/m/d H:i:s", strtotime($off->{$sic->SIC . '_submitted'})) : '-' ?> 
                            <br>
                            Updated : <?= $off->{$sic->SIC . '_updated'} ? date("Y/m/d H:i:s", strtotime($off->{$sic->SIC . '_updated'})) : '-' ?> 
                        </td>
                    </tr>
...
<script>
    ...
    function verify_submit() {
        ...
        //var p = confirm('You are going to submit zero entry KPI, proceed? ');
        var dont = 0;
        var zerokks = 0;
        var zerokg = 0;
        var zerobt = 0;
        ...
        var kpikks = Number($("#total_kpi_kks").text());
        var invkks = Number($("[id='inventorykks']").text());
        var kpikg = Number($("#total_kpi_kg").text());
        var invkg = Number($("[id='inventorykg']").text());
        var kpibt = Number($("#total_kpi_bt").text());
        var invbt = Number($("[id='inventorybt']").text());

        if (kpikks > invkks) {
            alert('Exceeds on KKS, adjustment required.');
            dont = 1;
        } else if (kpikks === 0) {
            zerokks = 1;
        }
        ...
        if (kpikg > invkg) {
            alert('Exceeds on KG, adjustment required.');
            dont = 1;
        } else if (kpikg === 0) {
            zerokg = 1;
        }
        ...
        if (kpibt > invbt) {
            alert('Exceeds on BT, adjustment required.');
            dont = 1;
        } else if (kpibt === 0) {
            zerobt = 1;
        }
        ...
        if (dont === 0) {
            ...
            if (confirm('Are you sure you want to submit this State KPI for Verification ?')) {
                
                if(zerokks && zerokg && zerobt && zerokpi){
                    if(confirm('You are going to submit zero entry KPI, proceed? ')){
